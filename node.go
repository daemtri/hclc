...
const (
	...
	// CommentTagName 用于从结构体encode的时候增加注释
...
func encodeStruct(in reflect.Value) (ast.Node, *ast.ObjectKey, error) {
	...
	for i := 0; i < l; i++ {
		...
		if meta.comment != "" {
			item.LeadComment = &ast.CommentGroup{
				List: []*ast.Comment{&ast.Comment{Text: "# " + meta.comment}},
			}
		}
		list.Add(item)
	}
...
func extractFieldMeta(f reflect.StructField) (meta fieldMeta) {
	...
	meta.comment = f.Tag.Get(CommentTagName)
